...
async function uploadBlobsToOss(blobs: Blob[]): Promise<string[]> {
  ...
  for (let i = 0; i < blobs.length; i++) {
    const file = new File([blobs[i]], `yuezhitong_export_${Date.now()}_${i}.png`, {
  ...
  const downloadBlobs = useCallback(async () => {
    try {
      for (let i = 0; i < blobs.length; i++) {
        ...
        a.download
          = blobs.length === 1
            ? `yuezhitong_conversation_${taskId}.png`
            : `yuezhitong_${taskId}_${i + 1}.png`
    ...
    try {
      ...
      const description
        = t('publishShareDescription')
          || 'I generated this conversation on yuezhitong using agent, check it out!'
      const tags = ['yuezhitong', 'agent']
