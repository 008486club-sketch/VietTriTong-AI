...
function Logo() {
  return (
    <Link href="/welcome" className="mr-8 flex items-center gap-2 md:mr-16">
      <Image
        src={logo}
        alt="越智通AI平台"
        width={40}
      ...
      />
      <span className="text-lg font-semibold md:text-xl">越智通AI平台</span>
    </Link>
  )
}
...
export function Navbar() {
  ...
  const lng = pathname.startsWith('/vi') ? 'vi' : 'zh-CN'
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* 语言切换 */}
            <Link
              href={lng === 'vi' ? '/zh-CN/welcome' : '/vi/welcome'}
              className="hidden cursor-pointer px-3 py-2 text-sm font-medium text-foreground/60 transition-colors hover:text-foreground md:inline-flex"
            >
              {lng === 'vi' ? '中文' : 'VI'}
            </Link>
            <button
              ...
              className="hidden cursor-pointer rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-muted md:inline-flex"
            >
              {t('navbar.login')}
            </button>
            <button
        ...
        {isMenuOpen && (
          <div className="mx-auto mt-4 flex max-w-7xl flex-col gap-2 border-t border-border/50 pt-4 md:hidden">
            ...
            <Link
              href={lng === 'vi' ? '/zh-CN/welcome' : '/vi/welcome'}
              className="cursor-pointer px-3 py-2 text-left text-sm font-medium text-foreground/60 transition-colors hover:text-foreground"
              onClick={() => setIsMenuOpen(false)}
            >
              {lng === 'vi' ? '中文' : 'Tiếng Việt'}
            </Link>
          </div>
        )}
      </nav>
